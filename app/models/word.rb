...
# 類義語(区分1)および対義語(区分2)の件数を取得する ※件数導出はviewで行うこと
  def cntSynm(kbn, wordId)
	if kbn == 1
	  Relation.find_by_sql(["select rel_id, syn_ant_cd, count(rel_id) as subTotal from relations where syn_ant_cd=? and rel_id in (select rel_id from relations where syn_ant_cd=? and word_id=?) group by rel_id", kbn, kbn, wordId])
	elsif kbn == 2
